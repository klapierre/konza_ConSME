...
for (i in 1:length(species_list)){
  ...
  if (length(missing_plot_years)>0){
...
coefficients_allsp <- array(NA, dim=c(length(sp_list), 3,12 ), dimnames= list(sp_list, c("coefficient", "P-val", "SE"),
...
if (dim(data_i[which(data_i$max_cover >0), ])[1]<50){                             
...
if (class(my_mod)[1]== "try-error" || lme4::isSingular(my_mod)) {  # if that model is singular or DN converge
  ...
  if (class(my_mod)[1] == "try-error" || length(which(summary(my_mod)$coefficients[,'Std. Error']>500))>0){# if that model DN fit or has wonky SE bars that indicate almost complete separation 
    my_mod <- try(
...
if (class(my_mod)[1] == "try-error") {
 # if the error message contains 0/1 separation term, then what has happened is there has been complete separation along the #trt condition-- you cannot quantify trt effects using a glm approach so you'll need to use a penalized regression approach
  if (grepl( "fitted rates numerically 0 or 1", attr(my_mod, "condition") , fixed= TRUE)) { # if the error message in the try-error object indicates complete separation
    stop("complete_sep_problems!")} else {
      stop("glm_problems!")}
  } else {
    if (i == 15){my_mod <- glmmTMB::glmmTMB(max_cover~watershed*trt + (1|block) + (1|year), data=data_i , family= nbinom1)} # lespedeza needed special help to converge
 all_mods[[length(all_mods) + 1]] <- my_mod
 if (i == 15){my_coefs <- summary(my_mod)$coefficient$cond} else {
...
names(my_coefs)[which(names(my_coefs) %in% c("trtBSI:watershedN4B", "trtBSX:watershedN4B", "trtXSI:watershedN4B", "trtXSX:watershedN4B", "trtXXI:watershedN4B"  ))] <- c("watershedN4B:trtBSI","watershedN4B:trtBSX", "watershedN4B:trtXSI", "watershedN4B:trtXSX", "watershedN4B:trtXXI"  ) # if the interaction term was written in the opposite order in the coefficient list, replace with standardized order
 if (i == 15){coef_present_wNA <- match(dimnames(my_coefs)[[1]], dimnames(coefficients_allsp)[[3]] )# which coefficients are actually present, put them in the correct order-- some might be missing
 coef_present <- na.omit(match(dimnames(my_coefs)[[1]], dimnames(coefficients_allsp)[[3]] ))# which coefficients are actually present, put them in the correct order-- some might be missing
 coefficients_allsp[i, "P-val",coef_present ] <- summary(my_mod)$coefficients$cond[which(!is.na(coef_present_wNA)),"Pr(>|z|)"]
 coefficients_allsp[i, "SE",coef_present ] <- summary(my_mod)$coefficients$cond[which(!is.na(coef_present_wNA)),"Std. Error"]
   
 } else {coef_present_wNA <- match(names(my_coefs), dimnames(coefficients_allsp)[[3]] )# which coefficients are actually present, put them in the correct order-- some might be missing
...
coefficients_allsp[i, "SE",coef_present ] <- summary(my_mod)$coefficients[which(!is.na(coef_present_wNA)),"Std. Error"] }
  coefficients_allsp[i, "coefficient",coef_present ] <-  my_coefs[which(!is.na(coef_present_wNA))]
 
} # ends the loop that says: if nothing works to fit this model, then add to problems list
...
length(sp_list[-badis])
#51/195 species had enough data to fit the model. 

# for the 51 species, which cofficients are significant and what is their sign?
...
sig.species <- left_join(sig.species, sp_unique, by= c("(species_names)"= "genus_species"))
length(which(sig.species$no.sig>1))
...
nsig.species <- left_join(nsig.species, sp_unique, by= c("(species_names)"= "genus_species"))
...
# make a figure of the distribution of significant coefficients----
coefficients_allsp_onlysig <- coefficients_allsp[-badis, 1,]
coefficients_allsp_onlysig[ which(coefficients_allsp[-badis, 2,]>0.05)] <- 0 #setting non-sig coefficients to zero 

coeffs_regressionsp <- as.data.frame(coefficients_allsp_onlysig)
coeffs_regressionsp$species <- species_list[-badis]
coeffs_regressionsp <- left_join(coeffs_regressionsp, sp_unique, by= c("species"= "genus_species"))

my_figs <- list()
trtnames <- c("trtBSI","trtBSX","trtXSI", "trtXSX"  ,"trtXXI")

# Pre-compute global x and y limits
all_vals <- c()
max_y <- 0
for (i in 1:length(trtnames)) {
  g <- coeffs_regressionsp[coeffs_regressionsp$lifeform== "g", trtnames[i]]
  f <- coeffs_regressionsp[coeffs_regressionsp$lifeform== "f", trtnames[i]]
  vals <- c(g, f)
  all_vals <- c(all_vals, vals)
  
  # temporary df for histogram to compute max counts
  df_tmp <- data.frame(value = vals)
  h <- hist(df_tmp$value, breaks = 10, plot = FALSE)
  max_y <- max(max_y, h$counts)
}

xlim_all <- range(all_vals, na.rm = TRUE)
ylim_all <- c(0, max_y)
my_x_labs <- c("All herbivores effect (BSI coefficients)", 
               "Bison and small mammal effect (BSX coefficients)", 
               NA,
               "Small mammal effect (XSX coefficients)",
               "Insect effect (XXI coefficients)"
               )
# Build plots
for (i in 1:length(trtnames)){
  g <- coeffs_regressionsp[coeffs_regressionsp$lifeform== "g", trtnames[i]]
  ...
  df <- data.frame(value = c(g, f),
                   group = c(rep("Grass", length(g)),
                             rep("Forb", length(f))))
  
  my_figs[[i]] <- ggplot(df, aes(x = value, fill = group)) +
    geom_histogram(position = "identity", alpha = 0.5, bins = 10) +
    scale_y_continuous(breaks = scales::pretty_breaks(n = 10),
                       limits = ylim_all, expand = c(0,0)) +  # shared y axis
...
# Only panel A (trtnames[5]) keeps the legend
my_figs[[5]] <- my_figs[[5]] +
  theme(legend.position = c(0.8, 0.8),
        legend.background = element_rect(fill = "white", color = NA))

# Panel letters in the order shown
letters <- c("A", "B", "C", "D")
panels <- list(my_figs[[5]], my_figs[[4]], my_figs[[2]], my_figs[[1]])
panels <- Map(function(p, lab) {
  p + annotation_custom(
    grob = textGrob(lab, x = unit(0, "npc"), y = unit(1, "npc"),
...
# make table K 
m_pasted <- (matrix(
  paste(round(coefficients_allsp[-badis,1,], 3), " (", round(coefficients_allsp[-badis,3,],3), ")", sep = ""),
  nrow = nrow(coefficients_allsp[-badis,1,]),
  ncol = ncol(coefficients_allsp[-badis,1,])
))
pvals_logical <- coefficients_allsp[-badis,2,]<= 0.05
m_pasted[which(pvals_logical)] <- paste0(m_pasted[which(pvals_logical)] ,"*", sep="")
m_pasted <- as.data.frame(m_pasted)
names(m_pasted) <- dimnames(coefficients_allsp[-badis,1,])[[2]]
m_pasted$Species <- dimnames(coefficients_allsp[-badis,1,])[[1]]
m_pasted$Species <- gsub("_", " ", m_pasted$Species)               # replace underscores
m_pasted$Species <- stringr::str_to_sentence(m_pasted$Species)
m_pasted <- m_pasted[, c(ncol(m_pasted), 1:(ncol(m_pasted)-1))]
m_pasted[m_pasted== "NA (NA)" ] <- NA
write.csv(m_pasted, file= "derived_data/04_Table_K_for_paper.csv")
...
simulationOutput <- DHARMa::simulateResiduals(all_mods[[i]] , plot = T, title= i)
mtext(paste("i= ",i, sep=""), side=3, line=20)
DHARMa::plotResiduals(simulationOutput, form = data_i$trt, title= i)
DHARMa::plotResiduals(simulationOutput, form = data_i$watershed)
}

par(mfrow= c(2, 2))
  for (i in 26:length(all_mods)){
